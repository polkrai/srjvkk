...
#                                rec.pertime) if privilege else 0.0
                record_amount += float(rec.qty) * float(rec.price)
...
#                'record_amount':ceil(float(record_amount)),
    ...
    def unpaid_records(self):
        ...
        for o in records:
            ...
            arrRecords.append({
                ...
                'subsidy_amount':float(subsidy_amount),
                'day_qty':math.ceil(qty / qtyPerDay),
				'reason': o.reason
            })
...
#                else:
                ...
                rItems[index]['amount'] += float(rd.total_amount)#ceil(float(rd.qty) * float(rd.price))
                ...
                rItems[index]['subsidy'] += float(rd.subsidy_amount)
...
#                    item_amount = (float(item.qty * item.price)) - item.subsidy_amount
                    ...
                    #c.total_subsidy += item_subsidy
                item_amount = float(rd.total_amount)#ceil(float(rd.qty) * float(rd.price))
                item_qty = rd.qty
                rItems.append({
                    'code': item_code,
                    'item': item_name,
        ...
        for item in rItems:
            ...
            item['amount'] = float(item['amount'])#math.ceil(item['amount'])
        ...
        if c.receipt.support_amount > 0:
            ...
            c.total_amount -= float(c.receipt.support_amount)
        if c.receipt.subsidy_amount > 0:
            c.discounts.append({
                'item':u'* ใช้สิทธิบัตร',
                'amount':c.receipt.subsidy_amount
            })
            c.total_amount -= float(c.receipt.subsidy_amount)
        if c.receipt.discount > 0:
            c.discounts.append({
                'item':u'* ส่วนลด',
                'amount':c.receipt.discount
            })
            c.total_amount -= float(c.receipt.discount)
        c.total_amount = float(c.total_amount)#math.ceil(c.total_amount)
