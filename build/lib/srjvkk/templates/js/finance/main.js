...
function loadImportantInfo(paId) {
    ...
    var AjaxObject = {
        ...
        handleFailure:function(o){
        	SR.dlgWait.hide();
        	showMessageDialog("Error", 'ไม่สามารถเชื่อมต่อกับ Server ได้ : ' + o.statusText, YAHOO.widget.SimpleDialog.ICON_ERROR);
        },

        processResult:function(o){
	
            SR.workspaceTab.tabpane.set('activeIndex', 0);
            ...
            try { 
                ...
                showMessageDialog("Error", "Invalid data", YAHOO.widget.SimpleDialog.ICON_ERROR); 
            } 
        },
...
function createWaitQueueGrid(obj, div_id, refresh_url_ds, refresh_url_query){
    ...
    obj.coldefs = [{key:"name", label:"ชื่อ - สกุล", width:180, resizeable:true}, {key:"queuetime", label:"เวลา", width:50}];
    obj.query = refresh_url_query
    obj.datasource = new YAHOO.util.DataSource(refresh_url_ds, {
		connXhrMode: "cancelStaleRequest",
        connMethodPost: true,
        responseType:YAHOO.util.DataSource.TYPE_JSON,
        responseSchema:{resultsList:"QueueResult.Queue", fields:["name", "vn", "queuetime", "q_id"]}
	});
            
    obj.datatable = new YAHOO.widget.DataTable(div_id, obj.coldefs, obj.datasource, {initialRequest:obj.query, scrollable:true, width:"280px", height:"250px"});
}
