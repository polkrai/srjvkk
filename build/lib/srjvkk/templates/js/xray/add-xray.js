...
function initButtons() {
    ...
    SR.btnCancel.addListener("click", function(e) { 
        ...
        window.location = "${h.url_for('/xray/xray')}" + 
    ...
    SR.btnSave.addListener("click", function(e) {
        ...
        if ($('is_done').checked) {
        	if (YAHOO.lang.trim($('film_number').value) == '') {
        		showMessageDialog("Error", 'กรุณากรอกจำนวนฟิล์ม',
                		YAHOO.widget.SimpleDialog.ICON_BLOCK);
        		return;
        	}
        }
        ...
        var cObj = YAHOO.util.Connect.asyncRequest('POST', "${h.url_for('/xray/save_xray')}", callback);
    });
}
