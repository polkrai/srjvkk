...
function initButtons() {
    ...
    SR.btnBack.addListener("click", function(e) {        
        window.location = "${h.url_for('/xray/setup_main')}";
    });
...
//            });
        ...
        var cObj = YAHOO.util.Connect.asyncRequest('POST', "${h.url_for('/xray/save_part')}", callback);
    });
        	...
        	if (SR.mode == 1) {
		    	...
		    	if (col.key == "del") {
	        		confirmDelete(this, rec, SR.delData,
	        				"ต้องการลบข้อมูลส่วนที่ต้องตรวจ: " + rec.getData("description"));
	        	}
        	}
        	if (SR.mode == 2) {
        		if (col.key == "del") {
