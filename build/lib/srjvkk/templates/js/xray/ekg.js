...
function initButtons() {
    ...
    SR.btnAdd.addListener("click", function(e) {        
    	...
    	window.location = "${h.url_for('/xray/add_ekg')}" + 
	...
	var table = new function(){
		this.coldefs =
            [{key:"id", label:"รหัส", hidden:true},
             {key:"ekg_date", label:"วันที่ส่งตรวจ", sortable:true, formatter: editFormatter},
             {key:"ekg_no", label:"เลขที่คลื่นหัวใจ"},
        	...
        	if (SR.mode == 1) {
		    	...
		    	if (col.key == "del") {
	        		confirmDelete(this, rec, SR.delData,
	        				"ต้องการลบข้อมูลใบส่งตรวจคลื่นหัวใจ: " + rec.getData("description"));
	        	}
        	}
        	if (SR.mode == 2) {
        		if (col.key == "del") {
        			...
        			SR.dlgConfirm = confirmDialog('ลบข้อมูล', "ต้องการลบข้อมูลใบส่งตรวจคลื่นหัวใจ: " + rec.getData("eeg_no"),
        					function() {
        						SR.dlgWait.show();
        						ajaxRequest('POST', "${h.url_for('/xray/delete_ekg')}" + "?ekg_id=" + rec.getData('id'),
        								{
        									success: function(o) {
        										SR.dlgWait.hide();
...
function editFormatter(elCell, oRecord, oColumn, oData) {    
	var url = "${h.url_for('/xray/add_ekg')}" + 
