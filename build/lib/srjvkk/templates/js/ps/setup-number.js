...
//            });
        ...
        var cObj = YAHOO.util.Connect.asyncRequest('POST', "${h.url_for('/psychosocial/save_number')}", callback);
    });
	...
	var table = new function(){
		this.coldefs =
            [{key:"id", label:"รหัส", hidden:true},
             {key:"number", label:"จำนวนครั้งที่เข้ารับการบำบัด", sortable:true, width:400,
            	 editor:new YAHOO.widget.TextboxCellEditor({disableBtns:true})},
             {key:"del", label:"", formatter: delPainter}];
        this.query = "";
        this.datasource = new YAHOO.util.DataSource(
            "${h.url_for('/psychosocial/get_numbers?')}",
        	...
        	if (SR.mode == 1) {
		    	...
		    	if (col.key == "del") {
	        		confirmDelete(this, rec, SR.delData,
	        				"ต้องการลบข้อมูลจำนวนครั้งที่เข้ารับการบำบัด: " + rec.getData("number"));
	        	}
        	}
        	if (SR.mode == 2) {
        		if (col.key == "del") {
