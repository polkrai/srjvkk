...
function initSendQueueDialog() {
    ...
    var handleSubmit = function() {
        ...
        var query = "q_id=" + SR.leftPane.waitQ.currentQId + "&src=social" +  "&comid=" + comid;
    ...
    SR.btnCallQueue.addListener("click", function(e) {        
        ...
        var callback =
        {
            success: function (o) {
                ...
                getPsycoTestReq(SR.leftPane.waitQ.currentVnId, SR.leftPane.waitQ.currentPaId, SR.workingWithCurrentQ);
            },
            failure: function (o) {
    ...
    var pane = new function(){
        ...
        this.tabpane.subscribe("activeIndexChange", function(event) {
        	...
        	if (event.newValue == 0) {
        		...
        		$('frame-psyco-appointment').src = url.psyco.appointment + param;

        	} 
			else if (event.newValue == 6) {
        		...
        		$('frame-psyco-reports').src = url.psyco.reports + param;
...
function getPsycoAppointmentResult(vnId, paId, orderId, qcalled) {
    //document.getElementById("frame-test-result").src = "${h.url_for('/psyco/test_result?vn_id="+ vnId + "&order_id=" + orderId + "')}";
	frm_appointment_result.reloadPage(vnId, paId, orderId, qcalled);
}
...
function initAllowedSendQueueDialog() {
    ...
    var handleSubmit = function() {
        ...
        var query = "q_id=" + SR.leftPane.waitQ.currentQId + "&src=psyco" +  "&action_id=" + $('select_allowed_action').options[actionindex].value;
        YAHOO.util.Connect.asyncRequest('POST', "${h.url_for('/srutil/transfer')}", callback, query);
        this.hide();
    };
    var handleCancel = function() {
