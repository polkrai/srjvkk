...
function initButtons() {
    ...
    SR.btnAdd.addListener("click", function(e) {        
        ...
        newRec = {id:-1, type:1, description:''};
...
//            });
        ...
        elData.value = jsData;
        var jsDelData = YAHOO.lang.JSON.stringify(SR.delProblems);
        var elDelData = document.getElementById("del-symptoms");
        elDelData.value = jsDelData; 
        
        var formObject = document.getElementById('form');
        YAHOO.util.Connect.setForm(formObject);        
        var cObj = YAHOO.util.Connect.asyncRequest('POST', "${h.url_for('/zone/save_symptom')}", callback);
    });
...
function initSymptomTable() {
	var table = new function(){
		this.typesArr = [{label:"แบบคัดกรองภาวะซึมเศร้า", value:1}, {label:"แบบคัดกรองความเสี่ยงต่อการฆ่าตัวตาย", value:2}];
		this.typeFormatter = function(elLiner, oRecord, oColumn, oData) {
			for (var i = 0; i < this.typesArr.length; i++) {
				if (oRecord.getData("type") == this.typesArr[i].value) {
					elLiner.innerHTML = this.typesArr[i].label;
					break;
				}
			}
        };
        ...
        this.coldefs =
            [{key:"id", label:"รหัส", hidden:true},
             {key:"description", label:"อาการและความรู้สึกนึกคิด", sortable:true, width:400,
            	 editor:new YAHOO.widget.TextareaCellEditor({disableBtns:false})},
             {key:"type", label: "ประเภทคำถาม", sortable:true, width: 400,
                 formatter: this.typeFormatter,
            	 editor: new YAHOO.widget.DropdownCellEditor({dropdownOptions: this.typesArr, 
            		 disableBtns:true })},
             {key:"del", label:"", formatter: delPainter}];
        this.query = "";
        this.datasource = new YAHOO.util.DataSource(
            "${h.url_for('/zone/get_symptoms?')}",
        	...
        	if (SR.mode == 1) {
		    	...
		    	if (col.key == "del") {
	        		confirmDelete(this, rec, SR.delProblems,
	        				"ต้องการลบข้อมูลอาการและความรู้สึกนึกคิด: " + rec.getData("description"));
	        	}
        	}
        	if (SR.mode == 2) {
        		if (col.key == "del") {
