...
function initButtons() {
    ...
    SR.btnAdd.addListener("click", function(e) {        
        ...
        newRec = {id:-1, description:'', order: SR.dataTable.datatable.getRecordSet().getLength() + 1};
...
//            });
        ...
        var cObj = YAHOO.util.Connect.asyncRequest('POST', "${h.url_for('/zone/save_objective')}", callback);
    });
	...
	var table = new function(){
		this.coldefs =
            [{key:"id", label:"รหัส", hidden:true},
             {key:"description", label:"วัตถุประสงค์การติดตามเยี่ยม", sortable:true, width:400,
        	...
        	if (SR.mode == 1) {
		    	...
		    	if (col.key == "del") {
	        		confirmDelete(this, rec, SR.delData,
	        				"ต้องการลบข้อมูลวัตถุประสงค์การติดตามเยี่ยม: " + rec.getData("description"));
	        	}
        	}
        	if (SR.mode == 2) {
        		if (col.key == "del") {
