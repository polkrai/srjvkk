...
//            });
        ...
        var cObj = YAHOO.util.Connect.asyncRequest('POST', "${h.url_for('/zone/save_problem')}", callback);
    });
	...
	var table = new function(){
		this.coldefs =
            [{key:"id", label:"รหัส", hidden:true},
             {key:"description", label:"สภาพปัญหาและความต้องการ", sortable:true, width:400,
        	...
        	if (SR.mode == 1) {
		    	...
		    	if (col.key == "del") {
	        		confirmDelete(this, rec, SR.delData,
	        				"ต้องการลบข้อมูลสภาพปัญหาและความต้องการ: " + rec.getData("description"));
	        	}
        	}
        	if (SR.mode == 2) {
        		if (col.key == "del") {
