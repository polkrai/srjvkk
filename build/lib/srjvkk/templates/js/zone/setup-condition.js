...
//            });
        ...
        var cObj = YAHOO.util.Connect.asyncRequest('POST', "${h.url_for('/zone/save_condition')}", callback);
    });
        	...
        	if (SR.mode == 1) {
		    	...
		    	if (col.key == "del") {
	        		confirmDelete(this, rec, SR.delData,
	        				"ต้องการลบข้อมูลเงื่อนไขการติดตามเยี่ยม: " + rec.getData("description"));
	        	}
        	}
        	if (SR.mode == 2) {
        		if (col.key == "del") {
