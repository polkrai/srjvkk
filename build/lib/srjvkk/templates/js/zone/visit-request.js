...
function initButtons() {
    ...
    SR.btnAdd.addListener("click", function(e) {        
    	...
    	window.location = "${h.url_for('/zone/add_visit_request')}" + "?pa_id=" + SR.currentPaId;// + "&vn_id=" + SR.currentVnId;
    });
}
...
function initDataTable() {
	...
	var table = new function(){
        ...
        this.datasource = new YAHOO.util.DataSource("${h.url_for('/zone/get_visit_requests')}", {
        		connXhrMode: "cancelStaleRequest",
        		connMethodPost: true,
        		responseType:YAHOO.util.DataSource.TYPE_JSON,
        		responseSchema:{resultsList:"data", fields:["id", "request_date", "symptom", "user", "request_no", "last_admit_date", "last_discharge_date", "result"]}});
        
        this.datatable = new YAHOO.widget.DataTable("data-table", this.coldefs, this.datasource, {scrollable:true, width:"100%", height:"400px", paginator:getPaginator(20, 20),initialRequest:this.query});
        ...
        this.handleCellClicked = function(oArgs) {
        	...
        	if (SR.mode == 1) {
		    	...
		    	if (col.key == "del") {
	        		...
	        		confirmDelete(this, rec, SR.delData, "ต้องการลบข้อมูลการขอให้ติดตามเยี่ยม: " + rec.getData("request_no"));
	        	}
        	}
...
function detailPainter(elCell, oRecord, oColumn, oData) {
	var url = "${h.url_for('/zone/add_visit_request')}" + "?id=" + oRecord.getData("id");
	elCell.innerHTML = "<a href='" + url +"'>" + oData + "</a>";
}

function resultPainter(elCell, oRecord, oColumn, oData) {
	var url = "${h.url_for('/zone/add_visit_result')}" + "?req_id=" + oRecord.getData("id") + "&pa_id=" + SR.currentPaId;
	var url2 = "${h.url_for('/zone/visit_result')}" + "?req_id=" + oRecord.getData("id") + "&pa_id=" + SR.currentPaId;
