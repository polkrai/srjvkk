...
from sqlalchemy.sql import and_, or_, not_, select, join, func, join, between
...
class ZoneController(BaseController):
    ...
    def get_refer_patients(self):
        ...
        #refers = s.query(model.ZoneRefer).filter_by(is_permitted=True, deleted=False).order_by(model.ZoneRefer.id.desc()).limit(500).all()
        #refers = s.query(model.ZoneRefer).filter(model.ZoneRefer.created_date <= start_date).filter(model.ZoneRefer.created_date >= end_date)
        refers = s.query(model.ZoneRefer).filter_by(deleted=False).filter(between(model.ZoneRefer.created_date, start_date, end_date)).order_by(model.ZoneRefer.id.desc()).all() #.filter(model.ZoneRefer.created_date.between(start_date, end_date)
    ...
    def get_visit_patients(self):
        ...
        visits = s.query(model.ZoneVisitRequest).filter_by(deleted=False).filter(between(model.ZoneVisitRequest.request_date, start_date, end_date)).order_by(model.ZoneVisitRequest.request_date.desc()).all()
        results = []
        for v in visits:
            results.append({
                'id': v.id,
...
#        c.icd10s = s.query(model.NeuralOrderIcd10).filter_by(order_id=c.order.id).order_by(model.NeuralOrderIcd10.priority).all() if c.order else []
    ...
    def diag_map(self):
        ...
        #start_date = model.DateUtil.parseDate(request.params.get('start_date'), True)
        #end_date = model.DateUtil.parseDate(request.params.get('end_date'), True)
        #.filter(model.NeuralIcd10.deleted==False)
        c.new_tab = request.params.get('new_tab') if request.params.get('new_tab') else ''
        
        c.diags = s.query(model.NeuralIcd10).filter_by(deleted=False).filter(model.NeuralIcd10.name!=None).\
            filter(model.NeuralIcd10.name!='').filter(func.length(model.NeuralIcd10.code)>=4).order_by(model.NeuralIcd10.code).all() 
        return render('/zone/diag_map.html')
        
	def diag_map_full(self):
		s = model.Session()
        
        c.diags = s.query(model.NeuralIcd10).filter_by(deleted=False).filter(model.NeuralIcd10.name!=None).\
        	filter(model.NeuralIcd10.name!='').filter(func.length(model.NeuralIcd10.code)>=4).order_by(model.NeuralIcd10.code).all()
        return render('/zone/diag_map_full.html')
        
    def get_diag_map_data(self):
        data = []
        diag = request.params.get('diag')
        stmt = select([model.NeuralOrderIcd10.visit_id, model.Patient.tmbpart, model.Patient.amppart, 
                       model.Patient.chwpart, model.ZoneTambon.tambon_name, model.ZoneTambon.lat, model.ZoneTambon.long], \
        ...
        for r in results: 
            data.append({
                'visit_id': r.visit_id,
                'tambon_name': r.tambon_name if r.tambon_name else '',
