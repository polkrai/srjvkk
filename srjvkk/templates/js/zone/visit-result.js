...
function initComponents(){
	...
	SR.queueSelectedCallback = {
		success: function() {
			...
			SR.dataTable.datasource.sendRequest("pa_id=" + SR.currentPaId + "&req_id=" + SR.reqId, {
	...
	var table = new function(){
		this.coldefs =
            [{key:"id", label:"รหัส", hidden:true},
             ...
             {key:"req_no", label:"เยี่ยมตามเลขรับเรื่อง"},
             ...
             {key:"is_rated", label:"ประเมินความพึงพอใจ?", formatter: "checkbox"},
             {key:"is_stop", label:"ยุติ?", formatter: "checkbox"},
             {key:"del", label:"", formatter: delPainter}];
        this.query = "pa_id=" + SR.currentPaId + "&req_id=" + SR.reqId;
        this.datasource = new YAHOO.util.DataSource(
            "${h.url_for('/zone/get_visit_results?')}",
            {connXhrMode: "cancelStaleRequest",
             connMethodPost: true,
             responseType:YAHOO.util.DataSource.TYPE_JSON,
             responseSchema:{resultsList:"data",
                fields:["id", "no", "visit_date", "score_9q", "score_8q", 
                        "req_no", "is_rated", "is_stop", "req_id"]}});
        	...
        	if (SR.mode == 2) {
        		if (col.key == "del") {
        			SR.dlgConfirm = confirmDialog('ลบข้อมูล', "ต้องการลบข้อมูลผลการติดตามเยี่ยมครั้งที่: " + rec.getData("no"),
        					function() {
        						SR.dlgWait.show();
        						ajaxRequest('POST', "${h.url_for('/zone/delete_visit_result')}" + "?result_id=" + rec.getData('id'),
        								{
        									success: function(o) {
        										SR.dlgWait.hide();
...
function editFormatter(elCell, oRecord, oColumn, oData) {
	var url = "${h.url_for('/zone/add_visit_result')}" +
		...
		"&pa_id=" + SR.currentPaId + "&req_id=" + oRecord.getData('req_id');
