...
//            });
        ...
        var cObj = YAHOO.util.Connect.asyncRequest('POST', "${h.url_for('/psychosocial/save_result')}", callback);
    });
...
function initDataTable() {
	var table = new function(){
        ...
        this.datasource = new YAHOO.util.DataSource(
            "${h.url_for('/psychosocial/get_results?')}",
        	...
        	if (SR.mode == 1) {
		    	...
		    	if (col.key == "del") {
	        		confirmDelete(this, rec, SR.delData,
	        				"ต้องการลบข้อมูลผลการบำบัด: " + rec.getData("description"));
	        	}
        	}
        	if (SR.mode == 2) {
        		if (col.key == "del") {
