...
function initButtons() {
    ...
    SR.btnBack.addListener("click", function(e) {        
        window.location = "${h.url_for('/psychosocial/setup_main')}";
    });
    ...
    SR.btnAdd.addListener("click", function(e) {        
        ...
        newRec = {id:-1, description:'', parent_id:null};
...
//            });
        ...
        var cObj = YAHOO.util.Connect.asyncRequest('POST', "${h.url_for('/psychosocial/save_method')}", callback);
    });
...
function initDataTable() {
	var table = new function(){
		this.parentFormatter = function(elLiner, oRecord, oColumn, oData) {
			for (var i = 0; i < SR.parentMethods.length; i++) {
				if (oRecord.getData("parent_id") == SR.parentMethods[i].value) {
					elLiner.innerHTML = SR.parentMethods[i].label;
					break;
				}
			}
        };
		...
		this.coldefs =
            [{key:"id", label:"รหัส", hidden:true},
             {key:"description", label:"รูปแบบการบำบัด", sortable:true, width:400,
            	 editor:new YAHOO.widget.TextareaCellEditor({disableBtns:false})},
        	 {key:"parent_id", label:"รูปแบบการบำบัดหลัก", sortable:true, width:400,
            		 formatter: this.parentFormatter,
                	 editor: new YAHOO.widget.DropdownCellEditor({dropdownOptions: SR.parentMethods, 
                		 disableBtns:true })},
             {key:"del", label:"", formatter: delPainter}];
        this.query = "parent_id=all";
        this.datasource = new YAHOO.util.DataSource(
            "${h.url_for('/psychosocial/get_methods')}",
            {connXhrMode: "cancelStaleRequest",
             connMethodPost: true,
             responseType:YAHOO.util.DataSource.TYPE_JSON,
             responseSchema:{resultsList:"methods",
                fields:["id", "description", "parent_id", "parent_desc"]}});
        	...
        	if (SR.mode == 1) {
		    	...
		    	if (col.key == "del") {
	        		confirmDelete(this, rec, SR.delData,
	        				"ต้องการลบข้อมูลรูปแบบการบำบัด: " + rec.getData("description"));
	        	}
        	}
        	if (SR.mode == 2) {
        		if (col.key == "del") {
...
function getParentMethods() {
	...
	var callback =
    {
        success:function(o) {
			try { 
				methods = YAHOO.lang.JSON.parse(o.responseText);
				SR.parentMethods = methods.methods;
				SR.btnAdd.set("disabled", false);
				SR.btnEdit.set("disabled", false);
				SR.dataTable = initDataTable();
	        } 
	...
	ajaxRequest('POST', "${h.url_for('/psychosocial/get_methods')}", callback, "parent_id=all");
}
