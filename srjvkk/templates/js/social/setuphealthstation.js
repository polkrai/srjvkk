...
//            });
        ...
        var jsHelp = YAHOO.lang.JSON.stringify(SR.stations);
        var dHelp = document.getElementById("stations");
        dHelp.value = jsHelp;
        var jsDelHelp = YAHOO.lang.JSON.stringify(SR.delStations);
        var dDelHelp = document.getElementById("del-stations");
        dDelHelp.value = jsDelHelp; 
        
        var formObject = document.getElementById('form');
        YAHOO.util.Connect.setForm(formObject);        
        var cObj = YAHOO.util.Connect.asyncRequest('POST', "${h.url_for('/social/save_health_station')}", callback);
    });
	...
	var stationTable = new function(){
        this.coldefs =
            ...
            //	 editor:new YAHOO.widget.TextboxCellEditor({disableBtns:true})},
             {key:"name", label:"รับการรักษา", sortable:true, minWidth:300,
            	 editor:new YAHOO.widget.TextboxCellEditor({disableBtns:true})},
             {key:"del",label:"", formatter: delPainter}];
        this.query = "";
        this.datasource = new YAHOO.util.DataSource(
            "${h.url_for('/social/get_health_stations?')}",
        	...
        	if (SR.mode == 1) {
		    	...
		    	if (col.key == "del") {
	        		confirmDelete(this, rec, SR.delStations,
	        				"ต้องการลบข้อมูลรับการรักษา: " + rec.getData("name"));
	        	}
        	}
        	if (SR.mode == 2) {
        		if (col.key == "del") {
