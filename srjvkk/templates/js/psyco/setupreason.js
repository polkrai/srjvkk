...
//            });
        ...
        var jsReason = YAHOO.lang.JSON.stringify(SR.reasons);
        var dReason = document.getElementById("reasons");
        dReason.value = jsReason;
        var jsDelReason = YAHOO.lang.JSON.stringify(SR.delReasons);
        var dDelReason = document.getElementById("del-reasons");
        dDelReason.value = jsDelReason; 
        
        var formObject = document.getElementById('form');
        YAHOO.util.Connect.setForm(formObject);        
        var cObj = YAHOO.util.Connect.asyncRequest('POST', "${h.url_for('/psyco/save_reason')}", callback);
	...
	var toolTable = new function(){
        this.coldefs =
            [{key:"id", label:"รหัส", hidden:true},
             {key:"name", label:"เหตุส่งบริการทางจิตวิทยา", sortable:true, minWidth:300, 
            	editor:new YAHOO.widget.TextboxCellEditor({disableBtns:true})},
             {key:"del", label:"", formatter: delPainter}];
        this.query = "";
        this.datasource = new YAHOO.util.DataSource(
            "${h.url_for('/psyco/get_reasons?')}",
        	...
        	if (SR.mode == 1) {
		    	...
		    	if (col.key == "del") {
	        		confirmDelete(this, rec, SR.delReasons,
	        				"ต้องการลบข้อมูลเหตุส่งบริการทางจิตวิทยา: " + rec.getData("name"));
	        	}
        	}
        	if (SR.mode == 2) {
        		if (col.key == "del") {
