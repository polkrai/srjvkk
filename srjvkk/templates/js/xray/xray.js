...
function initButtons() {
    ...
    SR.btnAdd.addListener("click", function(e) {        
    	...
    	window.location = "${h.url_for('/xray/add_xray')}" + "?pa_id=" + SR.currentPaId + "&vn_id=" + SR.currentVnId;
    });
}
	...
	var table = new function(){
		this.coldefs =
            [{key:"id", label:"รหัส", hidden:true},
             {key:"xray_date", label:"วันที่", sortable:true, formatter: editFormatter},
             {key:"xray_no", label:"เลขที่ X-ray", sortable:true},
             {key:"part", label:"สิ่งที่ต้องการตรวจ"},
             {key:"film_size", label:"ขนาดฟิล์ม"},
             {key:"film_number", label:"จำนวนฟิล์ม"},
        ...
        this.datasource = new YAHOO.util.DataSource(
            "${h.url_for('/xray/get_xrays?')}",
            {connXhrMode: "cancelStaleRequest",
             connMethodPost: true,
             responseType:YAHOO.util.DataSource.TYPE_JSON,
             responseSchema:{resultsList:"data",
                fields:["id", "xray_date", "xray_no", "part", 
                        "film_size", "film_number", "result",
                        "is_done", "done_date", "doctor_send"]}});
        ...
        this.handleCellClicked = function(oArgs) {
        	...
        	if (SR.mode == 2) {
        		if (col.key == "del") {
        			...
        			SR.dlgConfirm = confirmDialog('ลบข้อมูล', "ต้องการลบข้อมูลใบส่ง x-ray: " + rec.getData("xray_no"),
        					function() {
        						SR.dlgWait.show();
        						ajaxRequest('POST', "${h.url_for('/xray/delete_xray')}" + "?xray_id=" + rec.getData('id'),
        								{
        									success: function(o) {
        										SR.dlgWait.hide();
...
function editFormatter(elCell, oRecord, oColumn, oData) {
	...
	var url = "${h.url_for('/xray/add_xray')}" + "?id=" + oRecord.getData("id") + "&vn_id=" + SR.currentVnId + "&pa_id=" + SR.currentPaId;
	elCell.innerHTML = "<a href='" + url +"'>" + oData + "</a>";
}

function viewPainter(elCell, oRecord, oColumn, oData) {
	
	var url = "${h.url_for('/psychosocial/view_assessment')}" + "?id=" + oRecord.getData("id") + "&vn_id=" + SR.currentVnId + "&pa_id=" + SR.currentPaId;
...
function treatmentPainter(elCell, oRecord, oColumn, oData) {
	...
	if (oData != null) {
		var url  = "${h.url_for('/psychosocial/edit_patient_treatment')}" + "?ar_id=" + oRecord.getData("ar_id") + "&vn_id=" + SR.currentVnId + "&pa_id=" + SR.currentPaId;
		var url2 = "${h.url_for('/psychosocial/view_patient_treatment')}" + "?ar_id=" + oRecord.getData("ar_id") + "&vn_id=" + SR.currentVnId + "&pa_id=" + SR.currentPaId;;
...
function tResultPainter(elCell, oRecord, oColumn, oData) {
	...
	if (oRecord.getData('ar_id') != null) {
		...
		if (oData == null) {
			...
			var url = "${h.url_for('/psychosocial/edit_treatment_result')}" + "?ar_id=" + oRecord.getData("ar_id") + "&vn_id=" + SR.currentVnId + "&pa_id=" + SR.currentPaId;
			elCell.innerHTML = "<a style='float:right' href='" + url +"'><img src='/img/pencil.png'></a>";
		} 
		else {
			
			var url = "${h.url_for('/psychosocial/view_treatment_result')}" + "?ar_id=" + oRecord.getData("ar_id") + "&vn_id=" + SR.currentVnId + "&pa_id=" + SR.currentPaId;
...
function followupPainter(elCell, oRecord, oColumn, oData) {
	...
	if (oData) {
		var url = "${h.url_for('/psychosocial/view_treatment_followup')}" + "?ar_id=" + oRecord.getData("ar_id") + "&vn_id=" + SR.currentVnId + "&pa_id=" + SR.currentPaId;
		elCell.innerHTML = "<a href='" + url + "'>" + oData + "</a>";
	}
	
	if (oRecord.getData('treatment_result') != null) {
		url = "${h.url_for('/psychosocial/edit_treatment_followup')}" + "?ar_id=" + oRecord.getData("ar_id") + "&vn_id=" + SR.currentVnId + "&pa_id=" + SR.currentPaId;
		elCell.innerHTML += "<a style='float:right' href='" + url +"'><img src='/img/pencil.png'></a>";
	}
